// Libraries
import {
    Store as VuexStore, createStore, ActionContext, ActionTree,
    CommitOptions, DispatchOptions, GetterTree, MutationTree
} from "vuex";
...
const mutations: MutationTree<Array<InputState>> & Mutations = {
    [MutationTypes.SET_VALUE](newState: Array<InputState>, payload: InputPayload) {
        const property =
            newState.find((state: InputState) => state.Identifier === payload.Identifier);
...
export type Store = Omit<VuexStore<Array<InputState>>, "commit" | "getters" | "dispatch"> &
{
    commit<K extends keyof Mutations, P extends Parameters<Mutations[K]>[1]>
        (key: K, payload: P, options?: CommitOptions): ReturnType<Mutations[K]>;
} &
{
...
} &
{
    dispatch<K extends keyof Actions>
        (key: K, payload: Parameters<Actions[K]>[1], options?: DispatchOptions):
